...
def get_operation():
    global operation
    operation = int(
        input("Enter you considered operation. [1]>Add Student, [2]>Search Student: ")
    )
...
if operation == 1:
    ...
    physicgrd = input("Enter Student's physic grade: ")
    historygrd = input("Enter Student's history grade: ")
    csgrd = input("Enter Student's computer science grade: ")
    student_cinfo = courses(mathgrd, physicgrd, historygrd, csgrd)
    # merge informations
    all_info = {}
    ...
    try:
        try:
            user_file = open(
                "/Volumes/Files/Projects/Python-oop-database/data/database.txt", "a"
            )
        except:
            user_file = open(
                "/Volumes/Files/Projects/Python-oop-database/data/database.txt", "w"
            )
    ...
    try:
        database = open(
            "/Volumes/Files/Projects/Python-oop-database/data/database.txt", "r+"
        )

        all_students = database.readlines()
        for line in all_students:
            if len(line)<10:
                all_students.remove(line)
        ...
        for std in range(len(all_students)):
            ...
            if student["StudentID"] == studentid:
                report = {
                    ...
                    "physic grade: ": student["physics grade"],
                    "history grade: ": student["history grade"],
                    "computer science grade: ": student["computer science grade"],
                }
